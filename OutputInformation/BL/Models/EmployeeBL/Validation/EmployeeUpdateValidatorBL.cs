...
{
    public class EmployeeUpdateValidatorBL : IValidator<AcceptUpdateEmployeeDtoBL>
    {
        private readonly IDataContext context;

        public EmployeeUpdateValidatorBL(IDataContext context)
        {
            this.context = context;
        }


        public async Task Validate(AcceptUpdateEmployeeDtoBL dto)
        {
            if (dto is null)
            ...
            if (await Task.Factory.StartNew(() => !this.context.Set<Employee>().AsNoTracking().ToList().Exists(x => x.Id == dto.Id)))
